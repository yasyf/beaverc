...
namespace BC {
    ...
    struct Instruction
    {
        ...
        std::string toString() {
            static std::string names[] = {"LoadConst", "LoadFunc", "LoadLocal", "StoreLocal", "LoadGlobal", "StoreGlobal", "PushReference", "LoadReference", "StoreReference", "AllocRecord", "FieldLoad", "FieldStore", "IndexLoad", "IndexStore", "AllocClosure", "Call", "Return", "Add", "Sub", "Mul", "Div", "Neg", "Gt", "Geq", "Eq", "And", "Or", "Not", "Goto", "If", "Dup", "Swap", "Pop", "GarbageCollect", "Label", "ThrowUninitialized"};
            if (operand0) {
                return names[static_cast<int>(operation)] + " " + std::to_string(operand0.value());
            }
            return names[static_cast<int>(operation)];
        };
    };
